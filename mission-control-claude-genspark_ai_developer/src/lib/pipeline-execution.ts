...
function mapClientToPipelineData(client?: Client | null): Record<string, string> {
  ...
  return {
    ...
    posting_frequency: client?.operationalDetails || '',
    platforms: '',
    content_goal: '',
...
// Get next pending task — respects phase gates (prior phase must be complete before advancing)
export function getNextTask(instance: PipelineInstance, pipeline: Pipeline): PipelineTask | null {
  for (const phase of pipeline.phases) {
    const phaseTasks = instance.tasks.filter((t) => t.phaseId === phase.id)
    const phaseHasPending = phaseTasks.some((t) => t.status === 'pending')
    const phaseHasBlocked = phaseTasks.some((t) => t.status === 'blocked')
    const phaseIsComplete = phaseTasks.every((t) => t.status === 'completed')

    if (phaseIsComplete) continue // this phase done — check next

    if (phaseHasBlocked) return null // phase is blocked, cannot advance

    if (phaseHasPending) {
      // Only return a pending task if no in-progress tasks exist in this phase
      // (avoids running parallel tasks when phase requires sequential execution)
      const inProgress = phaseTasks.find((t) => t.status === 'in-progress')
      if (inProgress) return null
      return phaseTasks.find((t) => t.status === 'pending') || null
    }

    return null // phase has in-progress tasks — wait
  }
